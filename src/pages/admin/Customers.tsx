...
const AdminCustomers = () => {
  ...
  const kpiCards = [
    { label: 'Total Customers', value: customerStats.total, icon: <Users style={{ width: 16, height: 16, color: '#0071DC' }} />, iconBg: '#E6F1FB' },
    { label: 'Active', value: customerStats.active, icon: <Users style={{ width: 16, height: 16, color: '#2E8B57' }} />, iconBg: '#EAF3DE' },
    { label: 'Inactive', value: customerStats.inactive, icon: <Users style={{ width: 16, height: 16, color: '#9AA0A6' }} />, iconBg: '#F0F4F8' },
    { label: 'Total Revenue', value: `₹${customerStats.totalRevenue.toLocaleString('en-IN')}`, icon: <Users style={{ width: 16, height: 16, color: '#FFC220' }} />, iconBg: '#FFF8E6' },
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 500, color: '#1A1A1A', margin: 0 }}>Customers</h1>
          <p style={{ fontSize: 13, color: '#9AA0A6', margin: '4px 0 0' }}>Manage your customer relationships</p>
        </div>
      </div>
        ...
        {kpiCards.map((card) => (
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <span style={{ fontSize: 11, fontWeight: 500, color: '#5F6368', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{card.label}</span>
              <div style={{ width: 30, height: 30, borderRadius: 8, background: card.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {card.icon}
              </div>
            </div>
            <div style={{ fontSize: 22, fontWeight: 600, color: '#1A1A1A' }}>{card.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{
        ...
        <select
          ...
          style={{
            height: 36, padding: '0 36px 0 12px', border: '1.5px solid #E0E3E7',
            borderRadius: 8, fontSize: 14, color: '#1A1A1A', background: '#FFFFFF',
      ...
      <div style={{ background: '#FFFFFF', border: '0.5px solid #E0E3E7', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #E0E3E7', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 17, fontWeight: 500, color: '#1A1A1A' }}>Customer List</span>
          <span style={{ fontSize: 12, color: '#9AA0A6' }}>{filteredCustomers.length} results</span>
        </div>
        <Table>
          <TableHeader>
            <TableRow style={{ background: '#F0F4F8' }} className="hover:bg-[#F0F4F8]">
              <TableHead style={{ padding: '10px 16px', fontSize: 12, fontWeight: 500, color: '#5F6368', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Customer</TableHead>
              <TableHead style={{ padding: '10px 16px', fontSize: 12, fontWeight: 500, color: '#5F6368', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Contact</TableHead>
              <TableHead style={{ padding: '10px 16px', fontSize: 12, fontWeight: 500, color: '#5F6368', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Location</TableHead>
              <TableHead style={{ padding: '10px 16px', fontSize: 12, fontWeight: 500, color: '#5F6368', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Orders</TableHead>
              <TableHead style={{ padding: '10px 16px', fontSize: 12, fontWeight: 500, color: '#5F6368', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Total Spent</TableHead>
              <TableHead style={{ padding: '10px 16px', fontSize: 12, fontWeight: 500, color: '#5F6368', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Last Order</TableHead>
              ...
              >
                <TableCell style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div>
                      <p style={{ fontSize: 14, fontWeight: 500, color: '#1A1A1A', margin: 0 }}>{customer.name}</p>
                      <p style={{ fontSize: 11, color: '#9AA0A6', margin: '2px 0 0', fontFamily: 'monospace' }}>{customer.id.substring(0, 8)}...</p>
                    </div>
                  </div>
                </TableCell>
                <TableCell style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#5F6368' }}>
                      <Mail style={{ width: 12, height: 12, color: '#9AA0A6' }} />
                      {customer.email}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#5F6368' }}>
                      <Phone style={{ width: 12, height: 12, color: '#9AA0A6' }} />
                      {customer.phone}
                    </div>
                  </div>
                </TableCell>
                <TableCell style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#5F6368' }}>
                    <MapPin style={{ width: 12, height: 12, color: '#9AA0A6' }} />
                    {customer.location}
                  </div>
                </TableCell>
                <TableCell style={{ padding: '12px 16px', fontSize: 14, color: '#1A1A1A', fontWeight: 500 }}>{customer.totalOrders}</TableCell>
                <TableCell style={{ padding: '12px 16px', fontSize: 14, color: '#1A1A1A', fontWeight: 500 }}>₹{customer.totalSpent.toLocaleString('en-IN')}</TableCell>
                <TableCell style={{ padding: '12px 16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#5F6368' }}>
                    <Calendar style={{ width: 12, height: 12, color: '#9AA0A6' }} />
                    {customer.lastOrder}
                  </div>
                </TableCell>
                <TableCell style={{ padding: '12px 16px' }}>
                ...
                <TableCell style={{ padding: '12px 16px', textAlign: 'right' }}>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="rounded-lg border-[#E0E3E7] shadow-lg bg-white p-1 min-w-[140px]">
                      <DropdownMenuItem className="rounded-md hover:bg-[#F8FBFF] cursor-pointer text-sm py-2 text-[#1A1A1A]">
