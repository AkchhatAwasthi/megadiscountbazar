...
const ProductDetail = ({
  ...
  const [availableCoupons, setAvailableCoupons] = useState<any[]>(initialCoupons || []);
  ...
  const fetchProduct = async () => {
    try {
      ...
      const d = data as any;
      setProduct(d);
      if (d.images?.length) setActiveImage(d.images[0]);
      ...
      if (d.category_id) {
        fetchRelatedProducts(d.category_id, d.id);
      }
      fetchProductCoupons(d.id);
    } catch (error) {
    ...
    <div className="bg-white min-h-screen font-inter selection:bg-[var(--color-brand-red)]/10">
      ...
      <div className="max-w-[1280px] mx-auto px-6 py-6 md:py-10">
         <div className="flex items-center gap-2 text-[13px] text-[var(--color-text-secondary)]">
            ...
            <Link href="/products" className="hover:text-[var(--color-brand-red)] hover:underline">Shop</Link>
            <ChevronRight size={14} />
        ...
        {relatedProducts.length > 0 && (
          <section className="mt-32">
             <div className="flex justify-between items-end mb-10">
                ...
                <Link href="/products" className="text-[14px] font-[600] text-[var(--color-brand-red)] hover:underline flex items-center gap-1">
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
                {relatedProducts.map((related: any) => (
                  <ProductCard 
                     key={related.id} 
                     product={related} 
                     onViewDetail={() => router.push(`/product/${related.sku || related.id}`)}
                  />
                ))}
             </div>
          </section>
